...
import com.example.pingpinge.DBKey.Companion.DB_COMMUNITY
import com.example.pingpinge.databinding.ActivityCreateCommunityBinding
...
class CreateCommunityActivty: AppCompatActivity() {
    ...
    private fun uploadNoticeBoard( // 게시글 업로드 함수
        ...
        val model = CommunityDB(title, content, imageUri, key) // 게시글 데이터 형식으로 받아온 값들을 model에 저장
        ...
        when (requestCode) { // 갤러리 실행 시 보낸 코드가 2020일 때
            2020 -> {
                ...
                if (uri != null) { // 선택한 사진이 null이 아니라면
                    binding.imgView.setImageURI(uri) // 게시글 만들기 화면에 사진 띄우기
